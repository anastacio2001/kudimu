...
const NewLoginScreen = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('[FRONTEND LOGIN] Resposta /auth/login:', data);

      if (data.success) {
        // Salvar token
        localStorage.setItem('token', data.data.token);
        console.log('[FRONTEND LOGIN] Token salvo:', data.data.token);
        ...
        console.log('[FRONTEND LOGIN] Resposta /auth/me:', userData);

        if (userData.success) {
          localStorage.setItem('user', JSON.stringify(userData.data));
          console.log('[FRONTEND LOGIN] Tipo de usuário:', userData.data.tipo_usuario);
          
          // Redirecionar baseado no tipo de usuário
          if (userData.data.tipo_usuario === 'admin') {
            console.log('[FRONTEND LOGIN] Redirecionando para /admin');
            navigate('/admin');
          } else if (userData.data.tipo_usuario === 'cliente') {
            console.log('[FRONTEND LOGIN] Redirecionando para /client/dashboard');
            navigate('/client/dashboard');
          } else if (userData.data.tipo_usuario === 'respondente') {
            console.log('[FRONTEND LOGIN] Redirecionando para /campaigns');
            // Respondentes (usuários normais) vão para campanhas disponíveis
            navigate('/campaigns');
          } else {
            console.log('[FRONTEND LOGIN] Redirecionando para /campaigns (fallback)');
            // Fallback para qualquer outro tipo
            navigate('/campaigns');
          }
        } else {
          console.log('[FRONTEND LOGIN] userData.success = false, salvando data.data');
          localStorage.setItem('user', JSON.stringify(data.data));
          navigate('/campaigns');
        }
      } else {
        console.log('[FRONTEND LOGIN] Login falhou:', data.error);
